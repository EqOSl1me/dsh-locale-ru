...
const DEFAULT_CLONE = process.env.DSH_HARNESS_CLONE ?? resolve(process.cwd())
...
/* ---- merge fallback dictionaries into the corpus ---- */

const corpusNsSet = new Set(Object.keys(corpus))

// Every namespace named by a literal register('ns') call anywhere in the clone.
// These are real namespaces even when the dictionary import failed.
const registerNsSet = new Set()
for (const f of allFiles) {
  if (!f.endsWith('.ts') && !f.endsWith('.tsx')) continue
  const t = readFileSync(f, 'utf8')
  for (const m of t.matchAll(/locale\.register\(\s*['"]([^'"]+)['"]/g)) registerNsSet.add(m[1])
}

for (const f of allFiles) {
  if (!f.endsWith('locales.ts')) continue
  const relFile = relToClone(f)
  const text = readFileSync(f, 'utf8')
  const en = parseExportedDict(text, 'en')
  if (!en) continue
  const zh = parseExportedDict(text, 'zh') ?? {}
  // Which namespace? Scope the search to the dictionary's OWN directory first:
  // one package can host several namespaces (ui-sidebar-documentpreview has
  // html/, markdown/, pdf/, image/ and code/ sub-namespaces), and pooling them
  // would attribute every dictionary to whichever name was found first.
  const ownDir = relFile.slice(0, relFile.lastIndexOf('/'))
  const pkgDir = relFile.split('/').slice(0, 3).join('/')
  let ns = null
  // Namespace consts are named like NS, Foo_NS, AGENT_PRESET_SETTINGS_NS.
  // The literal register() call site is authoritative: a package may hold
  // unrelated *_NS constants (e.g. a settings-store key) that would attribute
  // the dictionary to the wrong namespace.
  const nsRe = /(?:const|export const)\\s+\\w*NS\\s*=\\s*['"]([^'"]+)['"]/
  const regRe = /locale\.register\(\s*['"]([^'"]+)['"]/
  const dirOf = (p) => relToClone(p).slice(0, relToClone(p).lastIndexOf('/'))

  // Pass 1: the dictionary's own directory, WITHOUT recursing - a literal
  // register() in a sibling index.ts is the strongest evidence.
  // Pass 2: the whole package, for packages whose register lives at the root.
  const pick = (scope, recursive) => {
    for (const other of allFiles) {
      const d = dirOf(other)
      if (recursive ? !d.startsWith(scope) : d !== scope) continue
      const t2 = readFileSync(other, 'utf8')
      const m3 = regRe.exec(t2)
      if (m3) return m3[1]
    }
    for (const other of allFiles) {
      const d = dirOf(other)
      if (recursive ? !d.startsWith(scope) : d !== scope) continue
      const m2 = nsRe.exec(readFileSync(other, 'utf8'))
      if (m2) return m2[1]
    }
    return null
  }

  const direct = pick(ownDir, false)
  if (direct !== null) {
    ns = direct
  } else {
    // Package-wide fallback. Exclude namespaces already owned by a more specific
    // subdirectory, or one dictionary steals a sibling's namespace (the
    // documentpreview code/ renderer has no register of its own and would
    // otherwise claim html/'s "documentHtml").
    const claimed = new Set()
    for (const other of allFiles) {
      if (!other.endsWith('locales.ts')) continue
      const od = dirOf(other)
      if (od === ownDir || !od.startsWith(pkgDir)) continue
      const t2 = readFileSync(other, 'utf8')
      if (!parseExportedDict(t2, 'en')) continue
      const c = pick(od, false)
      if (c !== null) claimed.add(c)
    }
    for (const other of allFiles) {
      if (!dirOf(other).startsWith(pkgDir)) continue
      const t2 = readFileSync(other, 'utf8')
      const m3 = regRe.exec(t2)
      if (m3 && !claimed.has(m3[1])) { ns = m3[1]; break }
    }
    if (ns === null) {
      const m2 = pick(pkgDir, true)
      if (m2 !== null && !claimed.has(m2)) ns = m2
    }
  }
  if (ns === null) {
    // No NS const in the package: it is either already covered by a register()
    // call resolved through an import, or its dictionary cannot be attributed.
    // Skipping is safe - an unattributed dictionary would invent a bogus
    // namespace and corrupt the corpus.
    continue
  }
  // A namespace resolved from a literal register() call is trustworthy even
  // when the register pass could not import the dictionary: that is exactly the
  // case this fallback exists for. Only skip when the package gave us no such
  // evidence, so a bogus name can never be invented.
  if (!corpusNsSet.has(ns) && corpus[ns] === undefined) {
      if (!registerNsSet.has(ns)) continue
  }
  if (!corpus[ns]) corpus[ns] = {}
  let added = 0
  for (const [k, v] of Object.entries(en)) {
    if (typeof v !== 'string') continue
    if (corpus[ns][k] === undefined) {
      corpus[ns][k] = { en: v, zh: typeof zh[k] === 'string' ? zh[k] : '' }
      added++
    }
  }
  if (added > 0) {
    notes.push('[' + relFile + '] ns ' + JSON.stringify(ns) + ': ' + added + ' key(s) recovered by textual parse (dynamic import failed)')
  }
}
...
/* --------------------------------------------- fallback: typed locales.ts -- */

/**
 * Some packages annotate the dictionary (`export const en: Record<K, string> =`)
 * and import types from a package that is not installed in a bare clone, so the
 * dynamic import fails. Parse those objects textually so their (often large)
 * dictionaries still land in the corpus.
 */
function grabBalancedObject(text, braceIdx) {
  let depth = 0
  let inStr = null
  let esc = false
  for (let i = braceIdx; i < text.length; i++) {
    ...
    if (c === '{') depth++
    else if (c === '}') { depth--; if (depth === 0) return text.slice(braceIdx, i + 1) }
  }
  return null
}

function parseExportedDict(text, locale) {
  const re = new RegExp('export const ' + locale + '[^=]*=\\s*\\{')
  const m = re.exec(text)
  if (!m) return null
  const braceIdx = m.index + m[0].length - 1
  const body = grabBalancedObject(text, braceIdx)
...
// Record which upstream revision this corpus describes: drift.mjs compares it
// against the installed dsh and must be able to flag a mismatch.
try {
  const { execFileSync } = await import('node:child_process')
  const git = (args) => execFileSync('git', ['-C', CLONE_ROOT, ...args], { encoding: 'utf8' }).trim()
  const tag = git(['describe', '--tags', '--always'])
  const revision = git(['rev-parse', 'HEAD'])
  let version = tag
  try {
    version = JSON.parse(readFileSync(join(CLONE_ROOT, 'package.json'), 'utf8')).version ?? tag
  } catch { /* keep the tag */ }
  writeFileSync(join(OUT_DIR, 'meta.json'), JSON.stringify({
    revision, tag, version,
    repo: 'https://github.com/deepseek-ai/deepseek-harness',
    extractedWith: 'scripts/extract.mjs',
    note: 'Corpus is the en/zh key inventory of every locale.register() call site at this revision. Re-extract after every dsh upgrade.',
  }, null, 2) + '\n')
} catch { /* not a git clone: leave any existing meta.json alone */ }
